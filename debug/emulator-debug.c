...
static Bool s_trace_stack[MAX_TRACE_STACK] = {TRUE};
...
Bool HOOK_emulator_step(Emulator* e, const char* func_name) {
    if (emulator_get_trace() && INTR.state < CPU_STATE_HALT) {
		printf("[%ld]", e->state.ticks);
        printf("[%x|%x|%x|%x]", read_u8_raw(e, 0xFF04), read_u8_raw(e, 0xFF05), 
								read_u8_raw(e, 0xFF06), read_u8_raw(e, 0xFF07));

        uint8_t opcode = read_u8_raw(e, REG.PC);
        printf("                               0x%02x\n", opcode);
        if (opcode == 0xCB) {
            uint8_t cbOP = read_u8_raw(e, REG.PC + 1);
            printf("[%ld]", e->state.ticks + 8);
	        printf("[%x|%x|%x|%x]", read_u8_raw(e, 0xFF04), read_u8_raw(e, 0xFF05), 
								read_u8_raw(e, 0xFF06), read_u8_raw(e, 0xFF07));
            printf("                               0x%02x\n", cbOP);
        }
